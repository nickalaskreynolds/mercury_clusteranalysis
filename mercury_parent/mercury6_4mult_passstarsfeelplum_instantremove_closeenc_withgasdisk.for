c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      MERCURY6_4BIN.FOR    (ErikSoft   4 May 2001)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
...
c
      ...
      integer opflag,ngflag,ndump,nfun,nbound
      ...
      real*8 tclus,mclus,rclus
      character*8 id(NMAX)
      character*80 outfile(3), dumpfile(5), mem(NMESS)
      external mdt_bs2, mdt_ra15
...
CNEWSTUFF#1,IZIDORO
      real*8 gdenstest,gastau,gaslife,gasscale
      real*8 t
      common/gas_data/gdenstest,gastau,gaslife,gasscale
...
c Get initial conditions and integration parameters
     ...
     %  en,am,cefac,ndump,nfun,nbod,nbig,nbound,m,xh,vh,s,rho,rceh,stat,
     %  id,epoch,ngf,opt,opflag,ngflag,outfile,dumpfile,lmem,mem,mclus,
     %  rclus,tclus)
c
...
c
     ...
     %  tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,nbound,m,xh,
     %  vh,mclus,rclus,tclus,s,rho,rceh,stat,id,ngf,opt,
     %  opflag,ngflag,outfile,dumpfile,mem,lmem,mdt_wb,mco_h2wb,
     %  mco_wb2h)
...
c Do a final data dump
     ...
     %  rmax,en,am,cefac,ndump,nfun,nbod,nbound,nbig,m,xh,vh,s,rho,
     %  rceh,stat,id,ngf,epoch,opt,opflag,dumpfile,mem,lmem,mclus,
     %  rclus,tclus)
c
c Calculate and record the overall change in energy and ang. momentum
  50  open  (23, file=outfile(3), status='old', access='append',
     %  err=50)
      ...
      call mxx_en (jcen,nbod,nbig,m,xh,vh,s,mclus,rclus,en(2),am(2))
c
...
c Input/Output
      integer nbod, nbig
      real*8 time,jcen(3),m(NMAX),x(3,NMAX),v(3,NMAX),a(3,NMAX)
c
c Local
      integer j
...
!     Doing GR perturbation (see. eq. 30 of Saha & Tremaine 1992)
      c = 173.144483 !speed of light in AU/day
      c2 = c*c
...
c
     ...
     %  rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,nbound,m,xh,vh,mclus,
     %  rclus,tclus,s,rho,rceh,stat,id,ngf,opt,opflag,
     %  ngflag,outfile,dumpfile,mem,lmem,onestep,coord,bcoord)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      ...
      integer lmem(NMESS),ndump,nfun,nbound
      real*8 time,tstart,tstop,dtout,h0,tol,jcen(3),rcen,rmax
      real*8 en(3),am(3),cefac,m(NMAX),xh(3,NMAX),vh(3,NMAX)
      real*8 s(3,NMAX),rho(NMAX),rceh(NMAX),ngf(4,NMAX)
      real*8 mclus,rclus,tclus
      character*8 id(NMAX)
      character*80 outfile(3),dumpfile(5),mem(NMESS)
c
c Local
      integer i,j,k,n,itmp,nclo,nhit,jhit(CMAX),iclo(CMAX),jclo(CMAX)
      integer dtflag,ejflag,stopflag,colflag,nstored,nextra
      integer nboundbig,nboundsmall
      ...
      real*8 PE,KE,totE,dx,dy,dz,vx,vy,vz,rpos,xpos,ypos,zpos
      real*8 mtot,r,r2,vtot,vtot2,rpos2,xcom(3),vcom(3)
      real*8 xb(3,NMAX),vb(3,NMAX)
c
c------------------------------------------------------------------------------
c
c Initialize variables. DTFLAG = 0/2: first call ever/normal
      ...
      nextra = nbod - nbound
      nboundbig = nbig - nextra
      nboundsmall = nbound - nboundbig
c
c Calculate close-encounter limits and physical radii
      call mce_init (tstart,algor,h0,jcen,rcen,rmax,cefac,nbod,
     %  nbound,nbig,m,xh,vh,s,rho,rceh,rphys,rce,rcrit,id,opt,
     %  outfile(2),1)
c
...
c Convert to internal coordinates and velocities
      call coord (time,jcen,nbound,nbig,nextra,h0,m,xh,vh,x,v,ngf,
     %     ngflag,opt,mclus)
c
c------------------------------------------------------------------------------
c
...
c Convert to heliocentric coordinates and output data for all bodies
        call bcoord (time,jcen,nbound,nbig,nextra,h0,m,x,v,xh,vh,ngf,
     %       ngflag,opt)
c Note I include my dummy Sun variable
...
c Save the current heliocentric coordinates and velocities
     ...
     %    opt)
        ...
        call bcoord(time,jcen,nbound,nbig,nextra,h0,m,x,v,xh0,vh0,ngf,
     %        ngflag,opt)
      end if
c
c     Dissipate cluster gas if it's time and recalculate accelerations
      if (time.gt.tclus) then 
         mclus = 0.d0
	 dtflag = 0
	 !make sure we only do this once by making tclus huge after this
	 tclus = time + h0 * 1.d50
      endif

      call onestep (time,tstart,h0,tol,rmax,en,am,jcen,rcen,nbod,nbig,
     %  nextra,m,x,v,mclus,rclus,tclus,s,rphys,rcrit,rce,
     %  stat,id,ngf,algor,opt,dtflag,ngflag,opflag,colflag,nclo,iclo,
...
c Reindex the surviving objects
	write(*,*) 'col!'
        call bcoord (time,jcen,nbound,nbig,nextra,h0,m,x,v,xh,vh,ngf,
     %        ngflag,opt)
        call mxx_elim (nbod,nbig,nbound,m,xh0,vh0,xh,vh,s,rho,rceh,
     %    rcrit,ngf,stat,id,mem,lmem,outfile(3),itmp)
c
...
c Check for collisions with the central body
     ...
     %        ngflag,opt)
      ...
      call mce_cent (time,h0,rcen,jcen,itmp,nbound,nbig,nextra,m,xh0,
     %  vh0,xh,vh,nhit,jhit,thit,dhit,algor,ngf,ngflag)
c
...
c Merge the object(s) with the central body
     ...
     %      vh,s,rphys,stat,id,opt,mem,lmem,outfile(3),mclus,rclus)
        end do
c
...
c Check for ejections
        itmp = 2
        if (algor.eq.11.or.algor.eq.12) itmp = 3
        call mxx_ejec (time,tstart,rmax,en,am,jcen,itmp,nbod,nbig,
     %    nextra,m,xh,vh,s,stat,id,opt,ejflag,outfile(3),mem,lmem,
     %    mclus,rclus)
c
...
c Input/Output
      integer nbod
      real*8 h,x0(3,NMAX), x1(3,NMAX), v0(3,NMAX),v1(3,NMAX)
...
c------------------------------------------------------------------------------
c
      subroutine mce_cent (time,h,rcen,jcen,i0,nbound,nbig,nextra,
     %  m,x0,v0,x1,v1,nhit,jhit,thit,dhit,algor,ngf,ngflag)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      ...
      real*8 time,h,rcen,jcen(3),m(NMAX),x0(3,NMAX),v0(3,NMAX)
      real*8 x1(3,NMAX),v1(3,NMAX),thit(CMAX),dhit(CMAX),ngf(4,NMAX)
c
c Local
      integer j,opt(8),nboundbig,nboundsmall,nbound,nextra
...
c
     ...
     %  vh,s,rphys,stat,id,opt,mem,lmem,outfile,mclus,rclus)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer i,j,nbod,nbig,stat(NMAX),opt(8),lmem(NMESS)
      real*8 time,tstart,elost,jcen(3)
      real*8 m(NMAX),xh(3,NMAX),vh(3,NMAX),s(3,NMAX),rphys(NMAX)
      real*8 mclus,rclus
...
c Input/Output
      integer nbod
      real*8 m(NMAX),x(3,NMAX),v(3,NMAX),hill(NMAX),a(NMAX)
c
c Local
      integer j
      real*8 r, v2, gm
...
c
     ...
     %  nbound,nbig,m,x,v,s,rho,rceh,rphys,rce,rcrit,id,opt,outfile,
     %  rcritflag)
c
      implicit none
      include 'mercury.inc'
...
c
c Input/Output
      integer nbod,nbig,algor,opt(8),rcritflag,nbound
      real*8 tstart,h,jcen(3),rcen,rmax,cefac,m(NMAX),x(3,NMAX)
      real*8 v(3,NMAX),s(3,NMAX),rho(NMAX),rceh(NMAX),rphys(NMAX)
      real*8 rce(NMAX),rcrit(NMAX)
      character*8 id(NMAX)
      character*80 outfile
c
c Local
      integer j,nboundbig,nboundsmall,nextra
...
c If required, calculate the changeover distance used by hybrid algorithm
	...
	!stars
	do j = nboundbig + 1,nbig
	  rcrit(j) = (2.D0 * m(j) * 5.D2 * h * 5.D2 * h)**(1.D0 / 3.D0)
	enddo
      end if
c
...
c
c Input/Output
      integer i, j, nbod, nbig, stat(NMAX)
      real*8 jcen(3),m(NMAX),xh(3,NMAX),vh(3,NMAX),s(3,NMAX),elost
      real*8 mclus,rclus
c
c Local
      integer k
...
c If a body hits the central body
      if (i.le.1) then
        call mxx_en (jcen,nbod,nbig,m,xh,vh,s,mclus,rclus,e0,l2)
c
...
c Calculate energy loss due to the collision
        call mxx_en (jcen,nbod,nbig,m,xh,vh,s,mclus,rclus,e1,l2)
        elost = elost + (e0 - e1)
      else
c
...
c
c Input/Output
      integer i0,nbod,nbig,ce(NMAX),nce,ice(NMAX),jce(NMAX)
      real*8 x0(3,NMAX),v0(3,NMAX),x1(3,NMAX),v1(3,NMAX),h,rcrit(NMAX)
c
c Local
      integer i,j
...
c
c Input/Output
      integer nbod,nbig,stat(NMAX),nowflag
      integer nclo,iclo(CMAX),jclo(CMAX)
      integer nhit,ihit(CMAX),jhit(CMAX),chit(CMAX),lmem(NMESS)
      real*8 time,h,rcen,m(NMAX),x0(3,NMAX),v0(3,NMAX)
      real*8 x1(3,NMAX),v1(3,NMAX),rce(NMAX),rphys(NMAX)
...
c Input/Output
      ...
      real*8 time,jcen(3),h,m(NMAX),x(3,NMAX),v(3,NMAX),xh(3,NMAX)
      real*8 vh(3,NMAX),ngf(4,NMAX)
c
c Local
      integer j
c
...
c------------------------------------------------------------------------------
c
      subroutine mco_wb2h (time,jcen,nbound,nbig,nextra,h,m,x,v,xh,vh,
     %  ngf,ngflag,opt)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod,nbig,ngflag,opt(8),nextra
      real*8 time,jcen(3),h,m(NMAX),x(3,NMAX),v(3,NMAX),xh(3,NMAX)
      real*8 vh(3,NMAX),ngf(4,NMAX)
c
c Local
      integer j,nbound,nboundsmall,nboundbig
      real*8 msum,mvsum(3),mxsum(3),temp,tmp2,tmp3
      real*8 xscom(3),vscom(3)
c
c------------------------------------------------------------------------------
c
      nboundbig = nbig - nextra
...
c     do binary positions and velocities
...
c
c     put passing stars in helio from COM
c     first solve for position and vel of sun in COM frame
      xscom(1) = x(1,nbig)
      xscom(2) = x(2,nbig)
      xscom(3) = x(3,nbig)
      vscom(1) = v(1,nbig)
      ...
      !big guys
      do j = 2,nboundbig 
c         xscom(1) = xscom(1) - m(j) * xh(1,j)
c         xscom(2) = xscom(2) - m(j) * xh(2,j)
c         xscom(3) = xscom(3) - m(j) * xh(3,j)
c         vscom(1) = vscom(1) - m(j) * vh(1,j)
c         vscom(2) = vscom(2) - m(j) * vh(2,j)
c         vscom(3) = vscom(3) - m(j) * vh(3,j)
      enddo
      !small guys
      do j = nbig + 1,nbig + nboundsmall
c         xscom(1) = xscom(1) - m(j) * xh(1,j)
...
c         vscom(3) = vscom(3) - m(j) * vh(3,j)
      enddo
      do j = nboundbig + 1, nbig
c         xscom(1) = xscom(1) - m(j) * x(1,j)
c         xscom(2) = xscom(2) - m(j) * x(2,j)
c         xscom(3) = xscom(3) - m(j) * x(3,j)
c         vscom(1) = vscom(1) - m(j) * v(1,j)
c         vscom(2) = vscom(2) - m(j) * v(2,j)
c         vscom(3) = vscom(3) - m(j) * v(3,j)
      enddo
c      xscom(1) = xscom(1) / (m(1) + m(2) + msum)
c      xscom(2) = xscom(2) / (m(1) + m(2) + msum)
c      xscom(3) = xscom(3) / (m(1) + m(2) + msum)
c      vscom(1) = vscom(1) / (m(1) + m(2) + msum)
c      vscom(2) = vscom(2) / (m(1) + m(2) + msum)
c      vscom(3) = vscom(3) / (m(1) + m(2) + msum)
c     now subtract the sun's x and v
      do j = nboundbig + 1, nbig
         xh(1,j) = x(1,j) - xscom(1)
         xh(2,j) = x(2,j) - xscom(2)
         xh(3,j) = x(3,j) - xscom(3)
         vh(1,j) = v(1,j) - vscom(1)
         vh(2,j) = v(2,j) - vscom(2)
         vh(3,j) = v(3,j) - vscom(3)
      enddo
c
c------------------------------------------------------------------------------
c
...
c Input/Output
      integer nbod,nbig,ngflag,opt(8)
      real*8 time,jcen(3),h,m(NMAX),xh(3,NMAX),vh(3,NMAX),x(3,NMAX)
      real*8 v(3,NMAX),ngf(4,NMAX)
c
c Local
      integer j
      real*8 mtot,temp
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      MCO_H2COM.FOR    (ErikSoft   2 March 2001)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: John E. Chambers
c
c Converts coordinates with respect to the central body to center-of-mass
c coordinates.
c
c------------------------------------------------------------------------------
c
      subroutine mco_h2com (time,jcen,nbod,nbig,m,xh,vh,x,v,mclus)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      ...
      real*8 time,jcen(3),m(NMAX),xh(3,NMAX),vh(3,NMAX),x(3,NMAX)
      real*8 v(3,NMAX),ngf(4,NMAX)
      real*8 mclus
c
c Local
...
c
      ...
      v(3,1) = temp * v(3,1)
c
c Calculate the center-of-mass coordinates and velocities
      do j = 2, nbod
        x(1,j) = xh(1,j) + x(1,1)
        x(2,j) = xh(2,j) + x(2,1)
        x(3,j) = xh(3,j) + x(3,1)
...
c------------------------------------------------------------------------------
c
      subroutine mco_h2wb (time,jcen,nbound,nbig,nextra,h,m,xh,vh,x,v,
     %  ngf,ngflag,opt,mclus)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbound,nbig,ngflag,opt(8),nextra
      real*8 time,jcen(3),h,m(NMAX),xh(3,NMAX),vh(3,NMAX),x(3,NMAX)
      real*8 v(3,NMAX),ngf(4,NMAX)
c
c Local
      integer j,nboundbig,nboundsmall
      real*8 msum,mxsum(3),mvsum(3),temp,tmp2,tmp3,msumall
      real*8 mxsumall(3),mvsumall(3),mclus
c
c------------------------------------------------------------------------------
c
      nboundbig = nbig - nextra
...
c     do other passing stars in inertial frame
      msumall = m(1) + m(2)! + mclus
      mxsumall(1) = m(2) * xh(1,2)
      mxsumall(2) = m(2) * xh(2,2)
      mxsumall(3) = m(2) * xh(3,2)
      mvsumall(1) = m(2) * vh(1,2)
      mvsumall(2) = m(2) * vh(2,2)
      mvsumall(3) = m(2) * vh(3,2)
      do j = 3,nbound+nextra
         msumall = msumall + m(j)
         mxsumall(1) = mxsumall(1) + m(j) * xh(1,j)
         mxsumall(2) = mxsumall(2) + m(j) * xh(2,j)
         mxsumall(3) = mxsumall(3) + m(j) * xh(3,j)
         mvsumall(1) = mvsumall(1) + m(j) * vh(1,j)
         mvsumall(2) = mvsumall(2) + m(j) * vh(2,j)
         mvsumall(3) = mvsumall(3) + m(j) * vh(3,j)
      enddo
      mxsumall(1) = mxsumall(1) / msumall
      mxsumall(2) = mxsumall(2) / msumall
      mxsumall(3) = mxsumall(3) / msumall
      mvsumall(1) = mvsumall(1) / msumall
      mvsumall(2) = mvsumall(2) / msumall
      mvsumall(3) = mvsumall(3) / msumall
      
      do j = nboundbig+1,nboundbig+nextra
         x(1,j) = xh(1,j)  -  mxsumall(1)
         x(2,j) = xh(2,j)  -  mxsumall(2)
         x(3,j) = xh(3,j)  -  mxsumall(3)
         v(1,j) = vh(1,j)  -  mvsumall(1)
         v(2,j) = vh(2,j)  -  mvsumall(2)
         v(3,j) = vh(3,j)  -  mvsumall(3)
      enddo

c     do particle velocities
      mvsum(1) = temp * mvsum(1)
      mvsum(2) = temp * mvsum(2)
      mvsum(3) = temp * mvsum(3)
      !big guys
...
c------------------------------------------------------------------------------
c
      subroutine mdt_bs2 (time,h0,hdid,tol,jcen,nbod,nbound,nbig,mass,
     %  x0,v0,s,rphys,rcrit,ngf,stat,dtflag,ngflag,opt,nce,ice,jce,
     %  force)
c
      implicit none
      include 'mercury.inc'
...
c
c Input/Output
      integer nbod, nbig, opt(8), stat(NMAX), dtflag, ngflag,nbound
      real*8 time,h0,hdid,tol,jcen(3),mass(NMAX),x0(3,NMAX),v0(3,NMAX)
      real*8 s(3,NMAX),ngf(4,NMAX),rphys(NMAX),rcrit(NMAX)
      integer nce,ice(nce),jce(nce)
      external force
c
c Local
...
c Calculate accelerations at the start of the step
      call force (time,jcen,nbod,nbound,nbig,mass,x0,v0,s,rcrit,a0,
...
c
        do j = 2, n
          call force (time,jcen,nbod,nbound,nbig,mass,xend,v0,s,rcrit,
     %      a,stat,ngf,ngflag,opt,nce,ice,jce)
          tmp0 = h * dble(j)
          do k = 2, nbod
            b(1,k) = b(1,k) + a(1,k)
            b(2,k) = b(2,k) + a(2,k)
...
c
        call force (time,jcen,nbod,nbound,nbig,mass,xend,v0,s,rcrit,a,
     %    stat,ngf,ngflag,opt,nce,ice,jce)
c
        do k = 2, nbod
          d(1,k,n) = xend(1,k)
...
c
     ...
     %  rcen,nbod,nbound,nbig,m,x,v,s,rphy,rcrit,rce,stat,id,ngf,algor,
     %  opt,ngflag,colflag,ce,nce,ice,jce,nclo,iclo,jclo,dclo,tclo,
     %  ixvclo,jxvclo,outfile,mem,lmem,force,mclus,rclus)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod,nbig,nce,ice(nce),jce(nce),stat(NMAX),ngflag,ce(NMAX)
      integer algor,opt(8),colflag,lmem(NMESS),nclo,iclo(CMAX)
      integer jclo(CMAX),nbound,nboundbig,nboundsmall,nextra
      real*8 time,tstart,h0,hrec,tol,rmax,elost,jcen(3),rcen
      real*8 m(NMAX),x(3,NMAX),v(3,NMAX),s(3,NMAX)
      real*8 rce(NMAX),rphy(NMAX),rcrit(NMAX),ngf(4,NMAX)
...
c Local
      ...
      integer i,j,k,nbs,nbsbig,statbs(NMAX),nbsbound
...
c Save old coordinates and integrate
     ...
     %    opt)
        call mdt_bs2 (time,hlocal,hdid,tol,jcen,nbs,nbsbound,nbsbig,mbs,
     %    xbs,vbs,sbs,rphybs,rcritbs,ngfbs,statbs,dtflag,ngflag,opt,nce,
     %    ibs,jbs,force)
        tlocal = tlocal + hdid
c
...
c
     ...
     %  nbig,nextra,m,x,v,mclus,rclus,tclus,s,rphys,rcrit,
     %  rce,stat,id,ngf,algor,opt,dtflag,ngflag,opflag,colflag,nclo,
     %  iclo,jclo,dclo,tclo,ixvclo,jxvclo,outfile,mem,lmem)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod,nbig,stat(NMAX),algor,opt(8),dtflag,ngflag,opflag
      integer colflag,lmem(NMESS),nclo,iclo(CMAX),jclo(CMAX)
      integer nextra,nbound,nboundbig,nboundsmall
      real*8 time,tstart,h0,tol,rmax,en(3),am(3),jcen(3),rcen
      real*8 m(NMAX),x(3,NMAX),v(3,NMAX),s(3,NMAX),rphys(NMAX)
      real*8 mclus,rclus,tclus
      real*8 rce(NMAX),rcrit(NMAX),ngf(4,NMAX),tclo(CMAX),dclo(CMAX)
      real*8 ixvclo(6,CMAX),jxvclo(6,CMAX)
      character*80 outfile(3),mem(NMESS)
      character*8 id(NMAX)
c
c Local
      ...
      integer nce2,ice2(NMAX),jce2(NMAX),ce2(NMAX)
...
c also the effective central mass and timestep for the binary Kepler drift
      if (dtflag.ne.2) then
        ...
        call mfo_wb (time,jcen,nbod,nbig,nextra,m,x,rcrit,a,stat,mclus,
     %       rclus,tclus)
        if (opt(8).eq.1) call mfo_user_wb (time,jcen,nbod,nbig,nbound,m,
     %     x,v,ausr)
        dtflag = 2
      end if
c
c Advance interaction Hamiltonian for H/2
      do j = 2, nbod
        v(1,j) = v(1,j)  +  hby2 * (a(1,j) + ausr(1,j))
        v(2,j) = v(2,j)  +  hby2 * (a(2,j) + ausr(2,j))
        v(3,j) = v(3,j)  +  hby2 * (a(3,j) + ausr(3,j))
      end do
c
...
c
c     Drift passing stars and Sun (in COM coords)
      do j = nboundbig+1,nboundbig+nextra
         x(1,j) = x(1,j) + v(1,j) * h0
         x(2,j) = x(2,j) + v(2,j) * h0
         x(3,j) = x(3,j) + v(3,j) * h0
      enddo
c
c Check whether any object separations were < R_CRIT whilst advancing H_K
c Look for close encounters between bodies that are bound
      call mce_snif_pl (h0,3,nbound,nbig,nextra,x0,v0,x,v,rcrit,ce,nce,
     %   ice,jce)
c Look for close encounters between stars
      call mce_snif_stars (h0,nboundbig+1,nbound,nbig,nextra,x0,v0,x,v,
     %   rcrit,ce2,nce2,ice2,jce2)
c Add in star encounters to arrays
      do j = nboundbig + 1, nbig
        ce(j) = ce2(j)
      enddo
      do j = nce+1,nce+nce2 
...
c If objects had close encounters, advance H_K using Bulirsch-Stoer instead
     ...
     %    nbod,nbound,nbig,m,x,v,s,rphys,rcrit,rce,stat,id,ngf,algor,
     %    opt,ngflag,colflag,ce,nce,ice,jce,nclo,iclo,jclo,dclo,tclo,
     %    ixvclo,jxvclo,outfile,mem,lmem,mfo_hkce,mclus,rclus)
      end if
c
c Advance solar Hamiltonian for H/2
      mvsum(1) = 0.d0
...
c
c Input/Output
      integer nbod,nbig,ngflag,stat(NMAX),opt(8),nce,ice(nce),jce(nce)
      real*8 time,jcen(3),m(NMAX),x(3,NMAX),v(3,NMAX),s(3,NMAX)
      real*8 a(3,NMAX),ngf(4,NMAX),rcrit(NMAX)
c
c Local
      integer j
...
c
c Input/Output
      integer nbod, nbig, stat(NMAX)
      real*8 jcen(3), m(NMAX), x(3,NMAX), a(3,NMAX), rcrit(NMAX)
c
c Local
      integer k
...
c Input/Output
      integer nbod, nbig, stat(NMAX)
      real*8 m(NMAX), x(3,NMAX), v(3,NMAX), a(3,NMAX)
c
c Local
      integer i, j
...
c------------------------------------------------------------------------------
c
      subroutine mfo_drct (i0,nbound,nextra,nbig,m,x,rcrit,a,
     %    stat)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer i0, nbound, nboundbig, nboundsmall, stat(NMAX)
      integer nbig,nextra
      real*8 m(NMAX), x(3,NMAX), a(3,NMAX), rcrit(NMAX)
c
c Local
      integer i,j
...
c
      do i = i0, nboundbig
c     do big-big interactions for bound objects
        do j = i + 1, nboundbig
          dx = x(1,j) - x(1,i)
          dy = x(2,j) - x(2,i)
          dz = x(3,j) - x(3,i)
...
c
c     do big-small interactions for bound objects
        do j = nbig+1, nbig + nboundsmall
          dx = x(1,j) - x(1,i)
          dy = x(2,j) - x(2,i)
          dz = x(3,j) - x(3,i)
...
c------------------------------------------------------------------------------
c
      subroutine mfo_wb (time,jcen,nbod,nbig,nextra,m,x,rcrit,a,stat,
     %     mclus,rclus,tclus)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod, nbig, stat(NMAX), nextra, nbound
      real*8 jcen(3), m(NMAX), x(3,NMAX), a(3,NMAX), rcrit(NMAX)
      real*8 mclus,rclus,tclus,time
c
c Local
      integer k,nboundbig,nboundsmall
...
c     do all force terms for extra stars
c     this also initializes all acc terms to zero
      call mfo_extras (nbound,nextra,nbig,m,x,rcrit,a,xs)

c     calculate accelerations due to cluster plummer potential
      call mfo_plummer (time,nbound,nextra,nbig,m,x,a,xs,mclus,rclus,
     %     tclus)
      ...
      xsm3 = s_1 * s_1 * s_1 
c
c Direct terms of bound particles (non-binary) on each other
      call mfo_drct (3,nbound,nextra,nbig,m,x,rcrit,a,stat)
c
c Add accelerations due to oblateness of the central body
      if (jcen(1).ne.0.or.jcen(2).ne.0.or.jcen(3).ne.0) then
        call mfo_obl (jcen,nbound,m,x,aobl,acen)
        do k = 3, nboundbig
          a(1,k) = a(1,k) + aobl(1,k) - acen(1)
          a(2,k) = a(2,k) + aobl(2,k) - acen(2)
          a(3,k) = a(3,k) + aobl(3,k) - acen(3)
        end do
        do k = nbig, nbig + nboundsmall
          a(1,k) = a(1,k) + aobl(1,k) - acen(1)
          a(2,k) = a(2,k) + aobl(2,k) - acen(2)
          a(3,k) = a(3,k) + aobl(3,k) - acen(3)
        end do
...
c
c     calculate binary acceleration
...
c------------------------------------------------------------------------------
c
      subroutine mfo_hkce (time,jcen,nbod,nbound,nbig,m,x,v,spin,rcrit,
     %  a,stat,ngf,ngflag,opt,nce,ice,jce)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod,nbig,stat(NMAX),ngflag,opt(8),nce,ice(nce),jce(nce)
      integer nbound,nboundbig,nboundsmall,nextra
      real*8 time,jcen(3),rcrit(NMAX),ngf(4,NMAX),m(NMAX)
      real*8 x(3,NMAX),v(3,NMAX),a(3,NMAX),spin(3,NMAX)
c
c Local
      integer i, j, k
...
c Input/Output
      integer nbod, nbig, stat(NMAX)
      real*8 jcen(3), m(NMAX), x(3,NMAX), xj(3,NMAX), a(3,NMAX)
c
c Local
      integer i,j,k,k1
...
c Input/Output
      integer nbod
      real*8 x(3,NMAX), v(3,NMAX), a(3,NMAX), ngf(4,NMAX)
c
c Local
      integer j
...
c Input/Output
      integer nbod
      real*8 jcen(3), m(NMAX), x(3,NMAX), a(3,NMAX), acen(3)
c
c Local
      integer i
...
c Input/Output
      integer nbod, nbig
      real*8 m(NMAX), x(3,NMAX), v(3,NMAX), a(3,NMAX), ngf(4,NMAX)
c
c Local
      integer j
c
...
c
c Input/Output
      integer nbod,nbig,opt(8),stat(NMAX),lmem(NMESS),stopflag
      integer nclo,iclo(nclo),jclo(nclo),nstored,ceflush
      real*8 time,tstart,rcen,rmax,m(NMAX),tclo(nclo),dclo(nclo)
...
c
     ...
     %  rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbound,nbig,m,x,v,s,
     %  rho,rceh,stat,id,ngf,epoch,opt,opflag,dumpfile,mem,lmem,
     %  mclus,rclus,tclus)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer algor,nbod,nbig,stat(NMAX),opt(8),opflag,ndump,nfun
      integer nbound
      integer lmem(NMESS)
      real*8 time,tstart,tstop,dtout,h0,tol,rmax,en(3),am(3)
      real*8 jcen(3),rcen,cefac,m(NMAX),x(3,NMAX),v(3,NMAX)
      real*8 s(3,NMAX),rho(NMAX),rceh(NMAX),ngf(4,NMAX),epoch(NMAX)
      real*8 mclus,rclus,tclus
...
c
c Dump the cluster info
        if (idp.eq.1) then
           c(1:12) = 'cluster.tmp '
 38        open (31, file=c(1:12), status='unknown', err=38)
        else
 39        open (31, file=dumpfile(4), status='unknown', err=39)
        endif
        write (31,315) mclus
        write (31,315) rclus
	write (31,315) tclus
...
c Infrequently-changed parameters
        ...
        write (33,*) mem(182)(1:lmem(182)),nbound
...
c
 ...
 315  format (1p,1(1x,e22.15),1x,i8)
      return
      end
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
c
     ...
     %  jcen,en,am,cefac,ndump,nfun,nbod,nbig,nbound,m,x,v,s,rho,rceh,
     %  stat,id,epoch,ngf,opt,opflag,ngflag,outfile,dumpfile,lmem,mem,
     %  mclus,rclus,tclus)
c
      implicit none
      include 'mercury.inc'
...
c Local
      ...
      character*80 infile(4),filename,c80
...
c Find out if this is an old integration (i.e. does the restart file exist)
      inquire (file=dumpfile(5), exist=oldflag)
c
...
c Read integration parameters
  ...
  40    lineno = lineno + 1
     ...
     %      lim(1,nsub)+1,mem(85),lmem(85))
        ...
        if (j.eq.22) read (c80,*,err=661) nbound
...
c Read in cluster parameters (Plummer Model mass, rad, and sun's position)
      filename = infile(4)
      if (oldflag) filename = dumpfile(4)
      inquire (file=filename, exist=test)
      if (.not.test) call mio_err (23,mem(81),lmem(81),mem(88),
     %    lmem(88),' ',1,filename,80)
      open (11, file=filename, status='old', err=668)
      read (11,*,err=668) mclus
      read (11,*,err=668) rclus
      read (11,*,err=668) tclus
      close (11)
...
c Read in energy and angular momentum variables, and convert to internal units
 330    open (35, file=dumpfile(5), status='old', err=330)
          read (35,*) opflag
...
c Calculate initial energy and angular momentum and write to information file
        ...
        s(2,1) = 0.d0
        s(3,1) = 0.d0
        call mxx_en (jcen,nbod,nbig,m,x,v,s,mclus,rclus,en(1),am(1))
...
c      if (opt(8).eq.1.and.(algor.eq.11.or.algor.eq.12)) call mio_err
c     %  (23,mem(81),lmem(81),mem(93),lmem(93),' ',1,mem(85),lmem(85))
      if (opt(8).eq.1.and.(algor.eq.11)) call mio_err
...
c Error reading epoch of Big bodies
 ...
 668  call mio_err (23,mem(81),lmem(81),mem(188),lmem(188),' ',1,
     %  ' ',1)
c
c------------------------------------------------------------------------------
c
...
c
c Input/Output
      integer nbod, nbig, stat(NMAX), opt(8), opflag, algor
      real*8 time,jcen(3),rcen,rmax,m(NMAX),xh(3,NMAX),vh(3,NMAX)
      real*8 s(3,NMAX),rho(NMAX)
      character*80 outfile
      character*8 id(NMAX)
c
c Local
...
c------------------------------------------------------------------------------
c
      subroutine mxx_ejec (time,tstart,rmax,en,am,jcen,i0,nbod,nbig,
     %  nextra,m,x,v,s,stat,id,opt,ejflag,outfile,mem,lmem,mclus,
     %  rclus)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer i0, nbod, nbig, stat(NMAX), opt(8), ejflag, lmem(NMESS)
      integer nextra,nboundbig,nbound,nboundsmall
...
c Calculate initial energy and angular momentum
      call mxx_en (jcen,nbod,nbig,m,x,v,s,mclus,rclus,e,l)
c
...
c
c Write message to information file
  30      open  (23,file=outfile,status='old',access='append',err=30)
          if (opt(3).eq.1) then
            call mio_jd2y (time,year,month,t1)
            flost = '(1x,a8,a,i10,1x,i2,1x,f8.5)'
            write (23,flost) id(j),mem(68)(1:lmem(68)),year,month,t1
...
c If ejections occurred, update ELOST and LLOST
      if (ejflag.ne.0) then
        call mxx_en (jcen,nbod,nbig,m,x,v,s,mclus,rclus,en(2),am(2))
        en(3) = en(3) + (e - en(2))
        am(3) = am(3) + (l - am(2))
      end if
c
...
c------------------------------------------------------------------------------
c
      subroutine mxx_elim (nbod,nbig,nbound,m,x0,v0,x,v,s,rho,rceh,
     %  rcrit,ngf,stat,id,mem,lmem,outfile,nelim)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod, nbig, nbound, nelim, stat(NMAX), lmem(NMESS)
      real*8 m(NMAX), x(3,NMAX), v(3,NMAX), s(3,NMAX)
      real*8 x0(3,NMAX), v0(3,NMAX)
      real*8 rho(NMAX), rceh(NMAX), rcrit(NMAX), ngf(4,NMAX)
      character*8 id(NMAX)
      character*80 outfile, mem(NMESS)
c
c Local
...
c------------------------------------------------------------------------------
c
      subroutine mxx_en  (jcen,nbod,nbig,m,xh,vh,s,mclus,rclus,e,l2)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod,nbig
      real*8 jcen(3),m(NMAX),xh(3,NMAX),vh(3,NMAX),s(3,NMAX),e,l2
      real*8 mclus,rclus
c
c Local
      integer j,k,iflag,itmp(8)
      real*8 x(3,NMAX),v(3,NMAX),temp,dx,dy,dz,r2,tmp,ke,pe,l(3)
      real*8 r_1,r_2,r_4,r_6,u2,u4,u6,tmp2(4,NMAX),a2
c
c------------------------------------------------------------------------------
c
      ke = 0.d0
...
c Convert to barycentric coordinates and velocities
      call mco_h2com(temp,jcen,nbod,nbig,m,xh,vh,x,v,mclus)
c
...
c
c Potential energy terms involving the plummer sphere
      a2 = rclus*rclus
      do j = 1, nbod
        dx = x(1,j)
        dy = x(2,j)
        ...
        if (r2.ne.0) pe = pe  -  mclus * m(j) / sqrt(r2 + a2)
      end do
c
...
c Input/Output
      integer nbod,nbig
      real*8 jcen(3),m(NMAX),xh(3,NMAX),vh(3,NMAX)
c
c Local
...
c
c Input/Output
      integer nbod,nbig,ngflag,opt(8),stat(NMAX)
      real*8 time,tstart,h0,tol,jcen(3),m(NMAX),x(3,NMAX),v(3,NMAX)
      real*8 s(3,NMAX),rceh(NMAX),rho(NMAX),epoch(NMAX),ngf(4,NMAX)
      character*8 id(NMAX)
c
c Local
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      MFO_EXTRAS.FOR    (25 March 2013)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: Nathan A. Kaib
c
c Figures out all force terms due to the extra passing cluster stars
c
c
c
c------------------------------------------------------------------------------
c
      subroutine mfo_extras (nbound,nextra,nbig,m,x,rcrit,a,xs)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbound,nextra,nbig,nboundsmall,nboundbig
      real*8 m(NMAX),x(3,NMAX),a(3,NMAX),xs(3),msum
      real*8 rcrit(NMAX)
c
c Local
      integer i,j,testinf(1)
      real*8 r,r2,r3,xsm3,force,s2,s_1,s_3
      real*8 dx,dy,dz,xxs(3),temp,tmp2,tmp3,faci,facj
      real*8 xscom(3),asun(3),atot(3),forcesum(3)
      real*8 binforcesum(3),sunforcesum(3),plforcesum(3)
      real*8 factor1,factor2,mprimandpl,mbound,xcom(3)

      real*8 rc,rc2,q,q2,q3,q4,q5,s
c
c------------------------------------------------------------------------------
c
      nboundbig = nbig - nextra
...
c     Initialize to zero
      ...
      atot(2) = 0.d0
      atot(3) = 0.d0
      do i = 1,nbound+nextra
         a(1,i) = 0.0d0
         a(2,i) = 0.0d0
         a(3,i) = 0.0d0
      enddo
c
c     Figure out position of Sun in inertial frame
      msum = 0.d0
c      xscom(1) = 0.d0 - (x(1,2) + xs(1)) * m(2)
c      xscom(2) = 0.d0 - (x(2,2) + xs(2)) * m(2)
c      xscom(3) = 0.d0 - (x(3,2) + xs(3)) * m(2)
c     do bound big guys
      do i = 3,nboundbig
c         xscom(1) = xscom(1) - x(1,i) * m(i)
c         xscom(2) = xscom(2) - x(2,i) * m(i)
c         xscom(3) = xscom(3) - x(3,i) * m(i)
         msum = msum + m(i)
      enddo
c     do bound small guys
      do i = nbig + 1, nbig + nboundsmall
c         xscom(1) = xscom(1) - x(1,i) * m(i)
c         xscom(2) = xscom(2) - x(2,i) * m(i)
c         xscom(3) = xscom(3) - x(3,i) * m(i)
      ...
      enddo
c     do unbound guys
      do i = nboundbig + 1, nboundbig + nextra
c         xscom(1) = xscom(1) - x(1,i) * m(i)
c         xscom(2) = xscom(2) - x(2,i) * m(i)
c         xscom(3) = xscom(3) - x(3,i) * m(i)
      enddo

c     we're gonna need these mass sums a lot
      mbound = m(1) + m(2) + msum
      mprimandpl = m(1) + msum

c     these are the positions of the Sun (primary) in COM
c      xscom(1) = xscom(1) / mbound
c      xscom(2) = xscom(2) / mbound
c      xscom(3) = xscom(3) / mbound

c     try something different...track Sun with tracer particle
      xscom(1) = x(1,nboundbig+nextra)
      xscom(2) = x(2,nboundbig+nextra)
      xscom(3) = x(3,nboundbig+nextra)

c     first figure out forces on binary from passing stars
      binforcesum(1) = 0.d0
      binforcesum(2) = 0.d0
      binforcesum(3) = 0.d0
      sunforcesum(1) = 0.d0
...
c     do passing stars + sun forces
         ...
         force = 0.d0 - m(i) * m(1) / r3
	 !this returns 0 if force is infinite
	 testinf = maxloc([force-1,force])-1

	 !only add finite forces
	 do j = 1,testinf(1)
	     sunforcesum(1) = sunforcesum(1) + force * dx
	     sunforcesum(2) = sunforcesum(2) + force * dy
	     sunforcesum(3) = sunforcesum(3) + force * dz

c     do direct terms for passing stars + sun on passing stars
	     a(1,i) = a(1,i) + force * dx / m(i)
	     a(2,i) = a(2,i) + force * dy / m(i)
	     a(3,i) = a(3,i) + force * dz / m(i)
	 enddo

c     do passing stars + boundbig forces
         do j = 3,nboundbig
            dx = x(1,i) - xscom(1) - x(1,j)
            dy = x(2,i) - xscom(2) - x(2,j)
            dz = x(3,i) - xscom(3) - x(3,j)
            r2 = dx*dx + dy*dy + dz*dz
            r = sqrt(r2)
            r3 = r*r2

            force = 0.d0 - m(i) * m(j) / r3
            plforcesum(1) = plforcesum(1) + force * dx
            plforcesum(2) = plforcesum(2) + force * dy
            plforcesum(3) = plforcesum(3) + force * dz

c     do direct terms for passing stars + planets on stars
            a(1,i) = a(1,i) - m(j) * dx / r3
            a(2,i) = a(2,i) - m(j) * dy / r3
            a(3,i) = a(3,i) - m(j) * dz / r3

c     do direct terms for passing stars + planets on planets
            a(1,j) = a(1,j) + m(i) * dx / r3
            a(2,j) = a(2,j) + m(i) * dy / r3
            a(3,j) = a(3,j) + m(i) * dz / r3
         enddo

c     do passing stars + boundsmall forces
         do j = nbig + 1, nbig + nboundsmall
            dx = x(1,i) - xscom(1) - x(1,j)
            dy = x(2,i) - xscom(2) - x(2,j)
            dz = x(3,i) - xscom(3) - x(3,j)
...
c     do passing stars + binary forces
         dx = x(1,i) - xscom(1) - x(1,2) - xs(1)
         dy = x(2,i) - xscom(2) - x(2,2) - xs(2)
         dz = x(3,i) - xscom(3) - x(3,2) - xs(3)
         r2 = dx*dx + dy*dy + dz*dz
         r = sqrt(r2)
         r3 = r*r2

         force = 0.d0 - m(i) * m(2) / r3
         binforcesum(1) = binforcesum(1) + force * dx
         binforcesum(2) = binforcesum(2) + force * dy
         binforcesum(3) = binforcesum(3) + force * dz

c     direct terms for passing stars + binary on binary
         a(1,2) = a(1,2) + m(i) * dx / r3
         a(2,2) = a(2,2) + m(i) * dy / r3
         a(3,2) = a(3,2) + m(i) * dz / r3

c     direct terms for passing stars + binary on stars
         a(1,i) = a(1,i) - m(2) * dx / r3
         a(2,i) = a(2,i) - m(2) * dy / r3
         a(3,i) = a(3,i) - m(2) * dz / r3
         
c     direct terms for passing stars on each other
         do j = i+1,nboundbig+nextra
            dx = x(1,i) - x(1,j)
            dy = x(2,i) - x(2,j)
            dz = x(3,i) - x(3,j)
            ...
            rc2 = rc * rc
c
c     decreasing direct forces if we have a close enc.
            if (s2.ge.rc2) then
              s_1 = 1.d0 / sqrt(s2)
              tmp2 = s_1 * s_1 * s_1
            else if (s2.le.0.01*rc2) then
...
c     add indirect terms to binary accel
      forcesum(1) = sunforcesum(1) + plforcesum(1) + binforcesum(1)
      forcesum(2) = sunforcesum(2) + plforcesum(2) + binforcesum(2)
      forcesum(3) = sunforcesum(3) + plforcesum(3) + binforcesum(3)
      a(1,2) = a(1,2) + forcesum(1) / mbound
      a(2,2) = a(2,2) + forcesum(2) / mbound
      a(3,2) = a(3,2) + forcesum(3) / mbound

c     add indirect terms to planets
      factor1 = (1.d0 + m(2) / mprimandpl) / mbound
      factor2 = 1.d0 / mprimandpl
c     do big bound particles first
      do i = 3,nboundbig
         a(1,i) = a(1,i) + forcesum(1) * factor1 - binforcesum(1) 
     %        * factor2
         a(2,i) = a(2,i) + forcesum(2) * factor1 - binforcesum(2)
     %        * factor2
         a(3,i) = a(3,i) + forcesum(3) * factor1 - binforcesum(3)
     %        * factor2
      enddo
c     do small bound particles next
      do i = nbig + 1, nbig + nboundsmall
         a(1,i) = a(1,i) + forcesum(1) * factor1 - binforcesum(1) 
     %        * factor2
         a(2,i) = a(2,i) + forcesum(2) * factor1 - binforcesum(2)
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      MFO_PLUMMER.FOR    (25 March 2013)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: Nathan A. Kaib
c
c Figures out all force terms due to tidal field of Plummer potential
c Note: this does not put Plummer accelerations on passing stars, just
c the bound bodies (Sun + planets + binary)
c
c
c
c------------------------------------------------------------------------------
c
      subroutine mfo_plummer (time,nbound,nextra,nbig,m,x,a,xs,
     %     mclus0,rclus,tclus)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbound,nextra,nbig,nboundbig,nboundsmall
      real*8 m(NMAX),x(3,NMAX),a(3,NMAX),xs(3),msum
      real*8 time
c
c Local
      ...
      real*8 binforcesum(3),sunforcesum(3),plforcesum(3)
      real*8 factor1,factor2,mprimandpl,mbound,xcom(3)
      real*8 mclus,rclus,rclus2,plumbot
      real*8 tclus,mclus0
c
c------------------------------------------------------------------------------
c
c
      !if (time.lt.tclus) then
      mclus = mclus0 * 1.d0
!      else 
!	 mclus = 0.d0
!      endif

      !mclus = mclus0 * exp(-time / tclus)
      rclus2 = rclus*rclus
      nboundbig = nbig - nextra
      nboundsmall = nbound - nboundbig

c     Figure out position of Sun in COM frame 
      msum = 0.d0
      !do bound big guys
      do i = 3,nboundbig
         msum = msum + m(i)
...
c     do accel on sun
      ...
      plumbot = (r2 + rclus2)**1.5
      
      force = mclus * r / plumbot * m(1)
      
c     Note: leaving this force positive since it will always
c     be used in back reactions
      sunforcesum(1) = sunforcesum(1) + force * dx / r
      sunforcesum(2) = sunforcesum(2) + force * dy / r
      sunforcesum(3) = sunforcesum(3) + force * dz / r

c     do accels on bound big guys
      do j = 3, nboundbig
         dx = xscom(1) + x(1,j)
         dy = xscom(2) + x(2,j)
         dz = xscom(3) + x(3,j)
         ...
         force = mclus * r / plumbot * m(j)

c     sum all planet forces for use later in indirect terms
         plforcesum(1) = plforcesum(1) + force * dx / r
         plforcesum(2) = plforcesum(2) + force * dy / r
         plforcesum(3) = plforcesum(3) + force * dz / r

c     do direct terms for plummer on planets
         a(1,j) = a(1,j) - mclus * dx / plumbot
         a(2,j) = a(2,j) - mclus * dy / plumbot
         a(3,j) = a(3,j) - mclus * dz / plumbot
      enddo
c     do accels on bound small guys
      do j = nbig+1, nbig + nboundsmall
         dx = xscom(1) + x(1,j)
         dy = xscom(2) + x(2,j)
         dz = xscom(3) + x(3,j)
...
c     do binary forces
      dx = xscom(1) + x(1,2) + xs(1)
      dy = xscom(2) + x(2,2) + xs(2)
      dz = xscom(3) + x(3,2) + xs(3)
      r2 = dx*dx + dy*dy + dz*dz
      r = sqrt(r2)

      plumbot = (r2 + rclus2)**1.5

      force = mclus * r / plumbot * m(2)

      binforcesum(1) = binforcesum(1) + force * dx / r
      binforcesum(2) = binforcesum(2) + force * dy / r
      binforcesum(3) = binforcesum(3) + force * dz / r

c     direct terms for plummer on binary
      a(1,2) = a(1,2) - mclus * dx / plumbot
      a(2,2) = a(2,2) - mclus * dy / plumbot
      a(3,2) = a(3,2) - mclus * dz / plumbot

c    do plummer forces on unbound stars
      do i = nboundbig + 1, nboundbig + nextra
	 dx = x(1,i)
	 dy = x(2,i)
	 dz = x(3,i)
         ...
         force = mclus * r / plumbot * m(i)

c     direct terms for plummer on unbound stars
         a(1,i) = a(1,i) - mclus * dx / plumbot
         a(2,i) = a(2,i) - mclus * dy / plumbot
         a(3,i) = a(3,i) - mclus * dz / plumbot
      enddo
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      PLUMMER_KICK.FOR    (25 March 2013)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: Nathan A. Kaib
...
c------------------------------------------------------------------------------
c
      subroutine plummer_kick (mclus,rclus,xclus,aclus)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      real*8 mclus,rclus,xclus(3),aclus(3)
c
c local
      real*8 force,plumbot,rpos2,rclus2,rpos
c
c------------------------------------------------------------------------------
c
c
      ...
      rclus2 = rclus*rclus
      rpos2 = xclus(1)*xclus(1) + xclus(2)*xclus(2) + xclus(3)*xclus(3)
      rpos = sqrt(rpos2)

      plumbot = (rpos2 + rclus2)**1.5

      force = 0.d0 - mclus * rpos / plumbot
      aclus(1) = force * xclus(1) / rpos
      aclus(2) = force * xclus(2) / rpos
      aclus(3) = force * xclus(3) / rpos
c
c------------------------------------------------------------------------------
c
      return
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c      MFO_USER_WB.FOR    (ErikSoft   2 March 2001)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: John E. Chambers
...
c------------------------------------------------------------------------------
c
      subroutine mfo_user_wb (time,jcen,nbod,nbig,nbound,m,x,v,a)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod, nbig, nbound,nboundbig,nboundsmall,nextra
      real*8 time,jcen(3),m(NMAX),x(3,NMAX),v(3,NMAX),a(3,NMAX)
c
c Local
      integer j
...
CNEWSTUFF#2,IZIDORO
      ...
      real*8 mass(NMAX)
      real*8 xh(NMAX),yh(NMAX),zh(NMAX)
      real*8 vxh(NMAX),vyh(NMAX),vzh(NMAX)
      real*8 axh(NMAX),ayh(NMAX),azh(NMAX)
      real*8 t, time_turb, TINY
      real*8 vxboundsum,vyboundsum,vzboundsum
      parameter(TINY = 4.0d-15)
...
c     we're going to rearrange the array to omit the binary star for 
c     Andre's disk forces. the initial runs will just have a dummy binary
c     but if we want to do a really binary, there will be indirect accleerations
c     because of the different coordinate systems
c     First, though, switch velocities to heliocentric velocities for bound particles
      !big bound guys
      vxboundsum = 0.0d0
      vyboundsum = 0.0d0
      vzboundsum = 0.0d0
      do j = 3, nboundbig
	 vxboundsum = vxboundsum + v(1,j) * mass(j) 
	 vyboundsum = vyboundsum + v(2,j) * mass(j)
	 vzboundsum = vzboundsum + v(3,j) * mass(j)
      enddo
      !small bound guys
      do j = nbig + 1, nbig + nboundsmall
	 vxboundsum = vxboundsum + v(1,j) * mass(j) 
	 ...
	 vzboundsum = vzboundsum + v(3,j) * mass(j)
      ...
      mass(1) = m(1)/K2 ! DO I REALLY NEED THAT? Yes because of gasdrag_kick
      xh(1)   = x(1,1)
      yh(1)   = x(2,1)
      zh(1)   = x(3,1)
      vxh(1)  = v(1,1) + vxboundsum / mass(1)
      vyh(1)  = v(2,1) + vyboundsum / mass(1)
      vzh(1)  = v(3,1) + vzboundsum / mass(1)
      axh(1)  = 0.0d0  ! initializing variables to be passed to gasdrag
      ayh(1)  = 0.0d0  ! otherwise you are f...
      azh(1)  = 0.0d0     
      !build array from big guys
      do j = 3, nboundbig !ignore binary star
        mass(j-1) = m(j)/K2 ! DO I REALLY NEED THAT? Yes because of gasdrag_kick
        xh(j-1)   = x(1,j)
        yh(j-1)   = x(2,j)
        zh(j-1)   = x(3,j)
        vxh(j-1)  = v(1,j) + vxboundsum / mass(1)
        vyh(j-1)  = v(2,j) + vyboundsum / mass(1)
        vzh(j-1)  = v(3,j) + vzboundsum / mass(1)
        axh(j-1)  = 0.0d0  ! initializing variables to be passed to gasdrag
        ayh(j-1)  = 0.0d0  ! otherwise you are f...
        azh(j-1)  = 0.0d0     
      end do
      !build array from small guys
      !need to adjust indexing to account for stars
      do j = nbig + 1, nbig + nboundsmall !ignore binary star
        mass(j-nextra-1) = m(j)/K2 ! DO I REALLY NEED THAT? Yes because of gasdrag_kick
        xh(j-nextra-1)   = x(1,j)
        yh(j-nextra-1)   = x(2,j)
        zh(j-nextra-1)   = x(3,j)
        vxh(j-nextra-1)  = v(1,j) + vxboundsum / mass(1)
        vyh(j-nextra-1)  = v(2,j) + vyboundsum / mass(1)
        vzh(j-nextra-1)  = v(3,j) + vzboundsum / mass(1)
        axh(j-nextra-1)  = 0.0d0  ! initializing variables to be passed to gasdrag
        ayh(j-nextra-1)  = 0.0d0  ! otherwise you are f...
        azh(j-nextra-1)  = 0.0d0     
      end do


      call gasdrag_kick(nbound-1,nboundbig-1,mass,xh,yh,zh,
     & vxh,vyh,vzh,axh,ayh,azh)

      do j = 3, nboundbig !no disk forces on passing stars
        if (mass(j-1).gt.TINY) then !to avoid NAN
          if(isnan(axh(j-1)).or.isnan(ayh(j-1)).or.isnan(azh(j-1)))then
	   write(*,*) "USER ACCELERATION BIG CRASH"
          stop
          endif
	   if(t.le.(gaslife))then !Be carefull with that!
             a(1,j)  = axh(j-1)   !I assume the gas lasts 3 Myr. There is better to do that but I am lazy
             a(2,j)  = ayh(j-1)
             a(3,j)  = azh(j-1)
	   else
             a(1,j)  = 0.0d0
      ...
      do j = nbig+1, nbig+nboundsmall !no disk forces on passing stars
        if (mass(j-nextra-1).gt.TINY) then !to avoid NAN
          if(isnan(axh(j-nextra-1)).or.isnan(ayh(j-nextra-1)).or.isnan(azh(j-nextra-1)))then
	   write(*,*) "USER ACCELERATION BIG CRASH"
          stop
          endif
	   if(t.le.(gaslife))then !Be carefull with that!
             a(1,j)  = axh(j-nextra-1)   !I assume the gas lasts 3 Myr. There is better to do that but I am lazy
             a(2,j)  = ayh(j-nextra-1)
             a(3,j)  = azh(j-nextra-1)
	   else
             a(1,j)  = 0.0d0
             a(2,j)  = 0.0d0
             a(3,j)  = 0.0d0
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c    MCE_SNIF_PL.FOR    (ErikSoft   3 October 2000)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: John E. Chambers
...
c------------------------------------------------------------------------------
c
      subroutine mce_snif_pl (h,i0,nbound,nbig,nextra,x0,v0,x1,v1,rcrit,
     %  ce,nce,ice,jce)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer i0,nbound,nbig,nextra,ce(NMAX),nce,ice(NMAX),jce(NMAX)
      real*8 x0(3,NMAX),v0(3,NMAX),x1(3,NMAX),v1(3,NMAX),h,rcrit(NMAX)
c
c Local
      integer i,j,nboundbig,nboundsmall
      real*8 d0,d1,d0t,d1t,d2min,temp,tmin,rc,rc2
      real*8 dx0,dy0,dz0,du0,dv0,dw0,dx1,dy1,dz1,du1,dv1,dw1
      real*8 xmin(NMAX),xmax(NMAX),ymin(NMAX),ymax(NMAX)
c
...
c
      ...
      if (i0.le.0) i0 = 2
      nce = 0
      do j = 2, nbound + nextra
        ce(j) = 0
      end do
c
c Calculate maximum and minimum values of x and y coordinates
      call mce_box (nbound + nextra,h,x0,v0,x1,v1,xmin,xmax,ymin,ymax)
c
c Adjust values for the Big bodies by symplectic close-encounter distance
      do j = i0, nboundbig
        xmin(j) = xmin(j) - rcrit(j)
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c    MCE_SNIF_STARS.FOR    (ErikSoft   3 October 2000)
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: John E. Chambers
...
c------------------------------------------------------------------------------
c
      subroutine mce_snif_stars (h,i0,nbound,nbig,nextra,x0,v0,x1,v1,
     %  rcrit,ce,nce,ice,jce)
c
      implicit none
      include 'mercury.inc'
c
...
c
c Local
      integer i,j,nbound,nboundbig,nboundsmall,nextra
      real*8 d0,d1,d0t,d1t,d2min,temp,tmin,rc,rc2
      real*8 dx0,dy0,dz0,du0,dv0,dw0,dx1,dy1,dz1,du1,dv1,dw1
      real*8 xmin(NMAX),xmax(NMAX),ymin(NMAX),ymax(NMAX)
c
...
c
c Calculate maximum and minimum values of x and y coordinates
      call mce_box (nbig,h,x0,v0,x1,v1,xmin,xmax,ymin,ymax)
c
c Adjust values for the Big bodies by symplectic close-encounter distance
      do j = i0, nbig
        xmin(j) = xmin(j) - rcrit(j)
...
CNEWSTUFF#3,IZIDORO
C My subroutines start from here! But, please see also mfo_user
c#####################################################################################################################################################
...
c...  Internals:
      ...
      real*8 v,po
      real*8 ax,ay,az 
      real*8 rsq,r,rad,ang,angold,erate

      real*8 rcomet,dcometcgs,dragcc,alpha
      real*8 zscale,gpower,deng0,Sig0
      real*8 a_ref
      real*8 TWOTHIRD,alpha_visc,ADIABATIC_INDEX,B_OVER_H
      real*8 temperature_index,TINY
      parameter(rcomet=50.) ! radius in Km for gas drag
      parameter(dcometcgs=1.5) ! body bulk density in g/cm3
      parameter(dragcc = 33.3291853141354/(rcomet*dcometcgs)) ! This corresponds to 3/(8 Rp pho_p) where Rp and pho_p are planetesimal's radius and bulk density, respectively
      ! note that dragcc is normalized according to the units of dcometcgs and the mass of the central body (in symba it takes the form dragcc = 8.4423d-1/(rcomet*dcometcgs))
      parameter (a_ref=1.06) ! position of Jupiter in FARGO scale (see gasdens.dat and gasvtheta.dat)
      parameter (TWOTHIRD = 0.66666666666666666d0)
      parameter(alpha_visc=0.002d0)  ! alpha viscosity
      parameter(ADIABATIC_INDEX=1.0d0) ! isothermal disk
      parameter(temperature_index=.5d0)
      parameter(B_OVER_H=0.4d0)
      parameter(TINY = 4.0d-15)

      real*8 znaught,sig,den,den0,eta,vkep,vc,vcx,vcy,wkep
      real*8 vrel2,vrel,mach,knudsen,cdrag,gdrag
      real*8 gdens(5000),gvth(5000),rg(5000)
      integer irtp(NMAX)
      real*8 gdens_init(5000),expmult,linmult
      real*8 typeax,typeay,typeaz,te,ti
 
      real*8 Gamma_0, massa, stellar_mass, aspect_ratio, sigma, radius, omega
      real*8 gamma_eff, zeta_eff,sigma_index
      real*8 X_S_PREFACTOR, x_s, k_p, p_nu,nu, p_chi
      real*8 lindblad_prefactor, lindblad_torque,torque_hs_ent
      real*8 torque_hs_baro, torque_c_lin_baro,corotation_torque
      real*8 e_f,ecc_corot,inc_corot,pe,ecc_lind,torque
      real*8 angular_momentum, lx,ly,lz,l
      real*8 time_mig, migration_factor, cs
      real*8 torque_c_lin_ent
      real*8 satura_f_nu,satura_f_chi
      real*8 cutoff_g_nu,cutoff_g_chi,cutoff_k_nu,cutoff_k_chi
      real*8 r_sat,delv_corec, v_2,ecc_fac

      integer passby ! make sure it calculated T1 to avoid use of 0 (inf.)
      real*8 temp1, temp2, temp3, temp4
      

      integer npt,ir,iinterp,ierr
      real*8 r_jup

      real*8 sh,a,e,inc,twave,sini,tT1e,tT1i,vth,xth,vrad,arad
      real*8 mtiny, tT1m,a0_sat,a0_jup

      data i1st/0/
      save i1st,iten,npt,gdens,gvth,rg,irtp,a0_sat,a0_jup
      ...
      zscale=0.05*5.2/5.2**1.25 ! avoids gfortran bug when initialized as parameter,  H/r= 5% at 5 AU (disk aspect ratio)
      typeax= 0.0d0
      typeay= 0.0d0
      typeaz= 0.0d0
      ax= 0.0d0
      ...
      if(i1st.eq.0)then
        npt=0
        open(unit=33,file='gasdens.dat')
        open(unit=34,file='gasvtheta.dat')
        do
          npt=npt+1
          read(33,*,end=100)rg(npt),gdens_init(npt)
          read(34,*,end=100)rg(npt),gvth(npt)
          rg(npt)=rg(npt)/a_ref
          gdens_init(npt)=abs(gdens_init(npt)/5.2**2*mass(1))    !abs solve a bug create when I smooth the surface density
          gvth(npt)=gvth(npt)*sqrt(K2)*sqrt(a_ref) 		 !v_kep in function of r/ref, sqrt(K2) is because  vk=sqrt((gm)/r). Recall that in fargo G=M=1 -> T=2pi, so it is necessary multiply by sqrt(G) to have in mercury units
        enddo                                                      
 100    close(33)
        close(34)
        i1st=1
        npt=npt-1
        write(*,*) 'Read gas files, total pts and pt500',npt,
     %    gdens_init(500)
        do i=1,nbod
          irtp(i)=0
        enddo

c...    Gaslifetime, GasTau and gasscale: gas dissipates exponentially following a characteristic e-folding timescale 
c...    gascale is used to scale the surface density by a factor 
        open(unit=35,file='gasparam.in')
        read(35,*) gaslife,gastau,gasscale
        write(*,*) 'gas parameters: disklife,tau,scale = ',gaslife,gastau,gasscale

        !gets jupiter and Saturn initial location to restore their orbits if necessary

        if(mass(2).ge.9.5d-4) then !Juptier
          temp1 = K2*(mass(1) + mass(2))
          call mco_x2el (temp1,xh(2),yh(2),zh(2),vxh(2),vyh(2),
     %      vzh(2), a,e,inc,temp2,temp3,temp4)
          a0_jup = a
        endif
        if(mass(3).ge.2.8d-4) then !Saturn
          temp1 = K2*(mass(1) + mass(3))
          call mco_x2el (temp1,xh(3),yh(3),zh(3),vxh(3),vyh(3),
     %      vzh(3), a,e,inc,temp2,temp3,temp4)
          a0_sat = a
        endif
      endif
      ...
      if(gastau.ne.0)then
        expmult=exp(-t/gastau)
c       linmult=1-(t/gastau)
      endif
      if (t.ge.gaslife)then
        expmult=0
      endif


      if(t.le.gaslife)then ! is gas still around?
c       write(*,*) "time gastau expmult",t,gastau,expmult
        do i=1,npt
          if(gaslife.ne.0)then 
            gdens(i)=gdens_init(i)*expmult
          else
            gdens(i)=0
          endif
        enddo
...
c        r_jup=sqrt(xh(2)**2+yh(2)**2) ! provided that Jupiter is body #2
 	r_jup = sqrt( xh(2)**2 + yh(2)**2  + zh(2)**2 )
        do i=2,nbod

          po=sqrt(xh(i)*xh(i)+yh(i)*yh(i))
          if(po.gt.0.1*r_jup.and.(po.lt.500*r_jup))then
          vkep=sqrt(K2*mass(1)/sqrt(xh(i)**2 + yh(i)**2 + zh(i)**2))
          wkep= vkep/po       !angular frequency
      
c         Scale height of the gas (Hayashi and co.)
          znaught=zscale*po**(1.25d0) !scale height (Hayashi et al.)

         
c         Density and azimuthal velocity by CLEVER interpolation
          iinterp=0
c          write(*,*)npt
          if(irtp(i).eq.0)then
            do ir=1,npt-1
              if(po.ge.rg(ir)*r_jup.and.po.lt.rg(ir+1)*r_jup)then
                sig=gdens(ir)+(gdens(ir+1)-gdens(ir))/
     %            (rg(ir+1)-rg(ir))/r_jup*(po-rg(ir)*r_jup)
                alpha = -(log(gdens(ir+1)) - log(gdens(ir)))/
     %            (log(rg(ir+1))    - log(rg(ir))) 
                vc=gvth(ir)+(gvth(ir+1)-gvth(ir))/
     %            (rg(ir+1)-rg(ir))/r_jup*(po-rg(ir)*r_jup)
                vc=vc/sqrt(r_jup)
                iinterp=1
                irtp(i)=ir
            ...
            enddo
 101        continue
          else ! remember the previous interpolation point and start from it
            if(po.ge.rg(irtp(i))*r_jup.and.po.lt.rg(irtp(i)+1)*r_jup)then
              ir=irtp(i)
              sig=gdens(ir)+(gdens(ir+1)-gdens(ir))/
     %          (rg(ir+1)-rg(ir))/r_jup*(po-rg(ir)*r_jup)
              alpha = -(log(gdens(ir+1)) - log(gdens(ir)))/
     ...
     %          (rg(ir+1)-rg(ir))/r_jup*(po-rg(ir)*r_jup)
              vc=vc/sqrt(r_jup)
              iinterp=1
            elseif(po.lt.rg(irtp(i))*r_jup)then
              do ir=irtp(i)-1,1,-1
                if(po.ge.rg(ir)*r_jup.and.po.lt.rg(ir+1)*r_jup)then
                  sig=gdens(ir)+(gdens(ir+1)-gdens(ir))/
     %              (rg(ir+1)-rg(ir))/r_jup*(po-rg(ir)*r_jup)
     ...
     %              (rg(ir+1)-rg(ir))/r_jup*(po-rg(ir)*r_jup)
              ...
              enddo
 102        continue
            elseif(po.ge.rg(irtp(i)+1)*r_jup)then
              do ir=irtp(i)+1,npt-1
                if(po.ge.rg(ir)*r_jup.and.po.lt.rg(ir+1)*r_jup)then
                  sig=gdens(ir)+(gdens(ir+1)-gdens(ir))/
     %              (rg(ir+1)-rg(ir))/r_jup*(po-rg(ir)*r_jup)
                  alpha = -(log(gdens(ir+1)) - log(gdens(ir)))/
 ...
 103          continue
          ...
          sig=sig*gasscale          !rescaling the surface density by a factor if necessary (see gasparam.in)


c          checking out the rescaling of surface density 
c           open(99,'compara.out')


          if(iinterp.eq.0)then
            write(*,*) "WARNING: particle out of gas range:"
            sig=0.d0
	    alpha=0.0d0
	    vc=0.0d0
            write(*,*) "THERE IS A BUG SOMEWHERE",rg(irtp(i)),rg(irtp(i)+1)
            stop             
          endif

          !gas density
          den0=sig/znaught/sqrt(PI)
          den=den0*exp(-zh(i)*zh(i)/(znaught*znaught))




c	  write (*,*) nbod, nbodm, i
          if (((nbod-1).gt.nbodm).and.(i.gt.nbodm).and.
     %      mass(i).gt.TINY)then !gasdrag only if bodies inside 200 AU (OUTER DISK EDGE in gasdens.dat if Jupiter is at 5 AU)
	
c           Gas moves slower than Kepler velocity
            vcx=-vc*yh(i)/po    !brakes vx
            vcy= vc*xh(i)/po    !brakes vy
            
            vrel2=(vxh(i)-vcx)*(vxh(i)-vcx) +
     &        (vyh(i)-vcy)*(vyh(i)-vcy)+vzh(i)*vzh(i)
            vrel=sqrt(vrel2)
            
            ! dimensionless
            ! vrel is in AU/day in mercury so weed take this into account when calculating mach
            mach = 1442.9*vrel*po**0.25d0 !Mach number   sound speed, cs = 1.2 km/s r**-0.25 !e.g. Hayashi 81 or Goodman and R. R. Rafikov, 2001, Eq 12
            ! Knudsen = lambda/R_pl, where lambda = mean free path and R_pl is the planetesimal radius
            ! lamda = 1/(n σ), where n is the the cross section σ  and the number density of  particles,  https://en.wikipedia.org/wiki/Mean_free_path
            ! σ is given in Adachi et al 1976 as 2x10^-15 cm^2
            ! n = (N_A / M) * pho_m (https://en.wikipedia.org/wiki/Number_density), where N_A is the avogrado number and M is the molar mass
            ! n = 1.6737e-9 g/(cm^2 pho_m) ! data from adachi et al 76 
            ! in our case pho_m  = den
            ! r comet is given in KM (normalization is made properly in the factor)
            ! alternatively see eq 21 in Brasser et al 2007
            
            ! in Symba knudsen is number is given by
            !knudsen = 1.1097308d-6/(den*rcomet) !Knudsen number  ! dimensionless

            ! in Mercury it is smaller by a factor of 4*PI*PI (solar mass in symba)
            knudsen = 2.8187e-8/(den*rcomet) !Knudsen number  ! dimensionless
            
            call dragcoef(mach,knudsen,cdrag)
            gdrag = dragcc*vrel2*den*cdrag
            
            ax = - gdrag * (vxh(i)-vcx)/vrel
            ay = - gdrag * (vyh(i)-vcy)/vrel
            az = - gdrag * vzh(i)/vrel


            ! if small body there is no tidal damping, only gas drag
	    typeax= 0.0d0
            typeay= 0.0d0
            typeaz= 0.0d0
         
cc..        write(*,*)"acceleratio for small body"   
          elseif(mass(i).lt.70*mass(1)/328000.0.and.po.le.200)then ! tidal damping only if smaller than 70 MEARTH and body inside 200 AU

            massa = mass(i) * K2
	    sigma = sig
	    sigma_index = alpha
	    stellar_mass = mass(1) * K2
	    aspect_ratio = znaught/po  !this is equal to sh
	    omega = wkep
	    cs = omega * znaught
	    nu = alpha_visc * cs * znaught
	    radius=sqrt(xh(i)**2 + yh(i)**2 + zh(i)**2)  ! talvez mudar isso para po!



            temp1 = K2*(mass(1) + mass(i))
            call mco_x2el (temp1,xh(i),yh(i),zh(i),vxh(i),vyh(i),
     %        vzh(i), a,e,inc,temp2,temp3,temp4)
              
            if(e.lt.1) then
              a = a/(1.0d0-e)
            endif

            if(a.gt.0.or.e.lt.1.0)then !ignore gas drag
 	      ! WE CALCULATE TOTAL TORQUE EXERTED BY THE DISK ON THE PLANET
  	      Gamma_0 = (massa / (stellar_mass * aspect_ratio))**2 
     %          * sigma * radius**4 * omega**2
  

c             We do not need this because it is an isothermal disk
c  	      gamma_eff = 2.d0 * Q_p * ADIABATIC_INDEX / (ADIABATIC_INDEX * Q_p + 0.5d0 * &
c 	      sqrt(2.d0 * sqrt((ADIABATIC_INDEX * ADIABATIC_INDEX * Q_p * Q_p + 1.d0)**2 - 16.d0 * Q_p * Q_p * (ADIABATIC_INDEX - 1.d0)) &
c  	      + 2.d0 * ADIABATIC_INDEX * ADIABATIC_INDEX * Q_p * Q_p - 2.d0))
  
	      gamma_eff = ADIABATIC_INDEX
 	      ...
 	      x_s=X_S_PREFACTOR/gamma_eff**0.25d0 * sqrt(massa / aspect_ratio)
  
  	      !------------------------------------------------------------------------------
  	      ! k_p is defined to limit the number of operation and to have a value independant from chi_p or nu_p
  	      k_p = radius * radius * omega * x_s * x_s * x_s / (2.d0 * PI)
  
c	      applies to non-isothermal disks
c	      chi= 1.5d0 * nu * ADIABATIC_INDEX * (ADIABATIC_INDEX - 1.d0) * &
c                      (1.5d0 + sqrt(3.d0) / tau + 1 / tau**2)
  
      	      !------------------------------------------------------------------------------
  	      p_nu = TWOTHIRD * sqrt(k_p / nu)

	  
	      p_chi=1.5d0*p_nu ! This does not matter for the isothermal case  our case here
  
  	      lindblad_prefactor = 
     %        -(2.5d0 + 1.7d0 * temperature_index - 0.1d0 * sigma_index) ! paardekooper, baruteau & kley 2010
 	      ...
 	      torque_hs_ent = 7.9d0 * zeta_eff / gamma_eff ! the factor (1 / Gamma_eff) is applied globally in the corotation torque
  	      ...
  	      torque_hs_baro = 1.1d0 * (1.5d0 - sigma_index) ! the factor (1 / Gamma_eff) is applied globally in the corotation torque
  	      torque_c_lin_baro = 0.7d0 * (1.5d0 - sigma_index) ! the factor (1 / Gamma_eff) is applied globally in the corotation torque

              call get_F(p_nu,satura_f_nu)
              call get_F(p_chi,satura_f_chi)
              call get_G(p_nu,cutoff_g_nu)
              call get_G(p_chi,cutoff_g_chi)
              call get_K(p_nu,cutoff_k_nu)
              call get_K(p_chi,cutoff_k_chi)


C  	      corotation_torque =
C     %         (1.d0 / gamma_eff)*(torque_hs_baro*satura_f_nu*cutoff_g_nu
C     %         + torque_c_lin_baro*(1 - cutoff_k_nu) 
C     %	        + torque_hs_ent*satura_f_nu * satura_f_chi
C     %         *  sqrt(cutoff_g_nu*cutoff_g_chi) 
C     %         + torque_c_lin_ent*sqrt((1 - cutoff_k_nu)*(1 - cutoff_k_chi)))


! isothermal case
              corotation_torque =
     %          (1.d0 / gamma_eff)*(torque_hs_baro*satura_f_nu*cutoff_g_nu
     %          + torque_c_lin_baro*(1 - cutoff_k_nu)
     %          + torque_c_lin_ent*(1 - cutoff_k_nu))
...
c     %	+ torque_c_lin_ent*sqrt((1 - cutoff_k_nu)*(1 - cutoff_k_chi)))
	      ...
	      call aei(xh,yh,zh,vxh,vyh,vzh,mass,i,a,e,inc)
c           formulae for corotation reduction FN14
c            if(a.gt.0.)then
              e_f= aspect_ratio/2.0d0 + 0.01
              ecc_corot=exp(-e/e_f)

	      inc_corot= 1- dtanh(inc/aspect_ratio)	
	
c	      formula for reduction of lindblad torque due to eccentricity
	      pe= 1 + (e/(2.25*aspect_ratio))**1.2 + (e/(2.84*aspect_ratio))**6
	      pe= pe / (1 - (e/(2.02*aspect_ratio))**4) 
	      ecc_lind= pe + (pe/dabs(pe))*(0.07*(inc/aspect_ratio)
     %          + 0.085*(inc/aspect_ratio)**4 
     % 	        - 0.08*(e/aspect_ratio)*(inc/aspect_ratio)**2)
              ecc_lind=1.0d0/ecc_lind
c

              torque =
     %        Gamma_0*
     %          (ecc_lind*lindblad_torque
     %          + ecc_corot*inc_corot*corotation_torque)


              !angular momentum
	      lx = yh(i)*mass(i)*vzh(i)  - zh(i)*mass(i)*vyh(i)
	      ly = zh(i)*mass(i)*vxh(i)  - xh(i)*mass(i)*vzh(i)
	      lz = xh(i)*mass(i)*vyh(i)  - yh(i)*mass(i)*vxh(i)
	      ...
	      time_mig = angular_momentum / torque
              ...
              migration_factor =  1.d0 / time_mig

c	      write(99,950) po,time_mig,alpha,torque,(1 - cutoff_k_nu)
c 950         format (5(X, e25.10))
c	      close(99)
        
              sh=znaught/po
              twave=mass(1)**2/mass(i)/sig/po**2*(sh)**4/
     %          (vc/po)

              sini=sin(inc)
              tT1e = twave/0.78*
     %          (1.-0.14*(e/sh)**2 + 0.06*(e/sh)**3
     %          + 0.18*(e/sh)*(sini/sh)**2)
              tT1i = twave/0.544*
     %          (1. - 0.30*(sini/sh)**2 +0.24*(sini/sh)**3.
     %          + 0.14*(sini/sh)*(e/sh)**2)
              
              !type I ecc and inc damping
              rsq = po*po					 !r_p^2 	
              rad = (xh(i)*vxh(i) + yh(i)*vyh(i))/rsq   !radial component velocity            
              ang = (xh(i)*vyh(i) - yh(i)*vxh(i))/rsq   !transversal component velocity         
              angold = ang - sqrt(K2*(mass(1)+mass(i))*po)/rsq  !vth-vk
              ax =  -(rad*xh(i) - angold*yh(i))/tT1e
              ay =  -(rad*yh(i) + angold*xh(i))/tT1e
              az =  -vzh(i)/tT1i

              !type I migration
	      if(migration_factor.eq.migration_factor)then ! make sure T1 is not infinity
	        typeax= vxh(i)*migration_factor
	        typeay= vyh(i)*migration_factor
	        typeaz= vzh(i)*migration_factor
              else
	        typeax= 0.0d0
                typeay= 0.0d0
                typeaz= 0.0d0
            ...
            else !fecha if a.gt.0 and e.gt.0 -> ignore gas drag and tidal damping
              ax= 0.0d0
              ay= 0.0d0
              az= 0.0d0
	      typeax= 0.0d0
          ...
          elseif ((i.eq.2.or.i.eq.3).and.mass(i) !type II ecc & inc damp on Jupiter and Saturn otherwise they get dynamically unstable
     %      .gt.90*mass(1)/328000.0)then


            ! see Peplinski et al 2008 for typeII migration timescale
            rsq = po*po


            !Lets apply tidal damping Jupiter and Saturn, using simple damping timescales (days)
            if(i.eq.2)then
              te= 1000 * 365.25 
              ti= 10000 * 365.25 
            else if (i.eq.3) then
            ! type III ten times faster than type II : ecc, fair to use a inc damping 2 or 3 orders magnitude faster than migration timescale
              te= 100 * 365.25 
              ti= 1000 * 365.25 
            endif

            ecc_fac= -2.d0 * (xh(i) * vxh(i) + yh(i) * vyh(i) 
     &        +  zh(i) * vzh(i) ) / (rsq * te)
          	

            ax = ecc_fac*xh(i) 
            ay = ecc_fac*yh(i)
            az = ecc_fac*zh(i) - vzh(i)/(ti)    !ten times slower than ecc damping

 
c 	    r_jup = sqrt( xh(2)**2 + yh(2)**2  + zh(2)**2 )
	    r_sat = sqrt( xh(3)**2 + yh(3)**2  + zh(3)**2 )


            !restoring Jupiter and SAturn original position
            ! this  mays sounds a bit artificial but it is consistent with hydro simulations, I think
            if(i.eq.2)then ! JUPITER MUST BE BODY 2
              v_2 = sqrt( vxh(i)**2 + vyh(i)**2 + vzh(i)**2 )
              delv_corec = sqrt(K2*mass(1)/r_jup)-sqrt(K2*mass(1)/a0_jup) ! Jupiter is Sitting around 5.25
              typeax = vxh(i) * 1.0d-4/365.25*delv_corec/v_2
              typeay = vyh(i) * 1.0d-4/365.25*delv_corec/v_2
              typeaz = vzh(i) * 1.0d-4/365.25*delv_corec/v_2
	    endif

            if(i.eq.3)then ! SATURN MUST BE BODY 3
              v_2 = sqrt( vxh(i)**2 + vyh(i)**2 + vzh(i)**2 )
              delv_corec = sqrt(K2*mass(1)/r_sat)-sqrt(K2*mass(1)/a0_sat)  ! Saturn is in the 3:2 with Jupiter
              typeax = vxh(i) * 1.0d-4/365.25*delv_corec/v_2
              typeay = vyh(i) * 1.0d-4/365.25*delv_corec/v_2
              typeaz = vzh(i) * 1.0d-4/365.25*delv_corec/v_2
            endif
          else !closing if testing over being a massive body or a small one 
            ax= 0.0d0
            ay= 0.0d0
            az= 0.0d0
	    typeax= 0.0d0
          ...
          ayh(i) = ay + typeay
          azh(i) = az + typeaz
          else !if outside disk range (see gasdens.dat). Remember it is normalzied by Jupiter's location
          axh(i) = 0.0
          ayh(i) = 0.0
          azh(i) = 0.0
          endif
        enddo ! closes loop over bodies
      endif ! closes if gas drag is still around (gaslife)
c      stop
      return
      end
...
c      implicit none
      ...
      !Local
      real*8 p_0 ! With tau_0 = 2
      real*8 idx_lt 
      real*8 idx_gt ! index for the case p greater than p_0
      ...
      parameter(p_0 = sqrt(8.d0 / (45.d0 * PI)))
      parameter(idx_lt = 1.5d0)
      parameter(idx_gt = 8.d0/3.d0)

      prefactor_lt = 16.d0 / (25.d0 * p_0**idx_lt)
      prefactor_gt = 9.d0 / 25.d0 * p_0**idx_gt


      
      if (p.lt.p_0) then
       y = prefactor_lt * p**idx_lt
      else
       y = 1.d0 - prefactor_gt / p**(idx_gt)
      end if
    
      return
      end 

      subroutine get_K(p,y)
  ! G function (30) of the paper : "A torque formula for non-isothermal Type I planetary migration - II Effects of diffusion"
  ! By Paardekooper, baruteau and Kley, (2010)
  ! Equation (30) is a particuliar case of the (29) equation with l=3/4 and tau_0=2
  !
...
c      implicit none
    ...
    !Local
      ...
      parameter(p_0 = sqrt(28.d0 / (45.d0 * PI))) ! With tau_0 = 7
      parameter(idx_lt = 1.5d0)
      parameter(idx_gt = 8.d0/3.d0) ! index for the case p greater than p_0
...
c**************************************************************
c     dragcoef.f Compute the drag coefficient C_D as a function of
c     Knudsen number and Mach number via the Reynolds number
c     The routine returns M, K, R and C_D.
c
c  Version from Martin on 7/11/07

       subroutine dragcoef(ma,kn,cdr)
...
c...  Executable code
       ...
       re = 4.4488239d0*ma/kn    !Reynolds number
c      write(*,*) "Here #K1.1",ma,kn
c      write(*,*) "Here #K1.2",re
       ...
       if (kn.lt.1.d0) then
           if(ma.ge.1.d0) then
             ...
             if(re.gt.1.d3) then
                cdr = 4.4d-1 + 1.56d0*ma*ma
             else
                cdr = 2.d0*ma*ma + (1.d0-ma*ma)*
     %               (2.4d1*(1.d0+1.5d-1*(re**0.687))/re)
             endif
           endif
       else              !If we're here, then kn is > 1.
         if(ma.lt. 1.8d0) then
	    cdr = 3.6/ma
	else
	    cdr = 2.d0
...
c**************************************************************
      subroutine aei(xh,yh,zh,vxh,vyh,vzh,mass,i,a,e,inc)
      implicit none
      include 'mercury.inc'
      real*8 mass(NMAX)
      real*8 xh(NMAX),yh(NMAX),zh(NMAX)
      real*8 vxh(NMAX),vyh(NMAX),vzh(NMAX)
      real*8 hx,hy,hz,h,h2,a,e,inc
      integer i
      real*8 r,energy,v2,gmsum,fac

      hx = yh(i)*vzh(i) - zh(i)*vyh(i)
      hy = zh(i)*vxh(i) - xh(i)*vzh(i)
      hz = xh(i)*vyh(i) - yh(i)*vxh(i)
      h2 = hx*hx + hy*hy +hz*hz
      h  = sqrt(h2)
      ...
      gmsum=mass(1)+mass(i)
      energy = 0.5d0*v2 - K2*gmsum/r
      if(energy .lt. 0.d0) then
	  a = -0.5d0*K2*gmsum/energy  
	  fac = 1.d0 - h2/(K2*gmsum*a)
	  if(fac.lt.0.)fac=0.d0
          e=sqrt(fac)
       else
          a=-1.
       endif
